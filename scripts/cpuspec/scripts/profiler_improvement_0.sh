...
do
	...
	export FILENAME=$val
	export THRESHOLD=$VAL
	runcpu --config=Native_jemalloc2k --noreportable --tune=base --copies=1 --iteration=1 --action=clobber --size=$SIZE $val >> output.txt
	runcpu --config=Native_jemalloc2k --noreportable --tune=base --copies=1 --iteration=$ITER --size=$SIZE $val >> output.txt
	
	runcpu --config=Scout_Prof_Imp --noreportable --tune=base --copies=1 --iteration=1 --action=clobber --size=$SIZE $val >> output.txt
	runcpu --config=Scout_Prof_Imp --noreportable --tune=base --copies=1 --iteration=$ITER --size=$SIZE $val >> output.txt
