...
do
	...
	runcpu --config=Native_jemalloc2k_mem_malloc --noreportable --tune=base --copies=1 --iteration=1 --action=clobber --size=$SIZE $val >> output.txt
	runcpu --config=Native_jemalloc2k_mem_malloc --noreportable --tune=base --copies=1 --iteration=$ITER --size=$SIZE $val >> output.txt
